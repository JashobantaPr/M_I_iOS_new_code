...
import 'package:incentivesgit/presentation/campaign_redeemed_point/service/campaign_redeemedpoint_service.dart';
import 'package:incentivesgit/presentation/homebottombar/campaignscreen/service/campaignservice.dart';

class CampaignRedeemedPointsController extends GetxController {
  final redeemedpoints = Get.arguments;
  var searchQuery = ''.obs;
  ...
  void transactionhistory(int? selectedYear, String? selectedCampaign) async {
    ...
    try {
      ...
      if (selectedYear != null) {
        result = await RedeemedPointService().transactiondata(selectedYear);
      } else {
        result = await RedeemedPointService().allTransactions();
      }
