...
import 'package:incentivesgit/presentation/campaign_expiredpoints_programId/service/expired_point_service.dart';

class CampaignExpiredPointsController1 extends GetxController {
  var searchQuery = ''.obs;
  ...
  void transactionhistory(int selectedYear) async {
    isLoading.value = true;
    try {
      var result = await ExpiredPointService().transactiondata(selectedYear);
      if (result.isNotEmpty) {
        var earnedTransactions = result
            .where((transaction) =>
                transaction.transactionType == 'wallet_expired')
            .toList();
  ...
  void transactionhistoryAll() async {
    isLoading.value = true;
    try {
      var result = await ExpiredPointService().transactiondataAll();
      if (result.isNotEmpty) {
        var earnedTransactions = result
            .where((transaction) =>
                transaction.transactionType == 'wallet_expired')
